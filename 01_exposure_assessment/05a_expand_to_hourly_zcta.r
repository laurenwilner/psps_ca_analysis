#-------------------------------------------------
# PSPS: Expand data back out to hourly (ZCTA)
...
if(washout == TRUE){
  psps_temp <- read_csv(paste0(intermediate_dir, "ca_zcta_daily_psps_washout_wf_2013-2022.csv"))
} else{
  psps_temp <- read_csv(paste0(intermediate_dir, "ca_zcta_daily_psps_no_washout_wf_2013-2022.csv"))
}
...
if(washout == TRUE){
    write.csv(psps_analysis_hourly, paste0(clean_dir, "ca_zcta_hourly_psps_washout_wf_2013-2022.csv"), row.names = FALSE)
} else{
    write.csv(psps_analysis_hourly, paste0(clean_dir, "ca_zcta_hourly_psps_no_washout_wf_2013-2022.csv"), row.names = FALSE)
}
