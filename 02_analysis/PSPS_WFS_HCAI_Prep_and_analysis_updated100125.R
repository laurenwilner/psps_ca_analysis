...
## updated by Chen Chen on 10/01/25 to use only serviced areas by the three elec companies providing PSPS info
## also added extrction of WF concentration by outcome days
...
hcai_dir <- "D:/OSHPD/Caitlin/CC TS/"
proj_dir <- "D:/Caitlin/PSPS/Data/"
out_dir <- "D:/Caitlin/PSPS/Results/"

## Data import--please check description of each dataset
ooi_long <- fread(file.path(proj_dir, "HCAI_ccts_psps-ooi_2013_2019.csv"), 
                  select = c("patzip", "serv_dt", "cc_id", "agecat", "sex", "race_grp",
                             "diag_p", "enc_type", "case_indicator", "OOI")) ## file containing health data for each zip_code-day
# age formatting
ooi_long[, agecat := fcase(
...
ooi_long <- ooi_long[agecat != "19 years and under"]

psps_levels <- read_csv(file.path(proj_dir, "ca_zcta_event_level_psps_no_washout_wf_classified_2013-2022-2.csv")) ## file containing psps event data
...
zipcode_served <- fread(file.path(proj_dir, "utility_zips.csv"))
zipcode_served <- zipcode_served[nchar(zipcode_served$ZIP_CODE)==5, ] ##1504

zipcode_served$respmissing[!zipcode_served$ZIP_CODE %in% unique(ooi_long$patzip[ooi_long$OOI=="resp"])] <- "Yes" ## 0 missing
zipcode_served$cardiomissing[!zipcode_served$ZIP_CODE %in% unique(ooi_long$patzip[ooi_long$OOI=="cardio"])] <- "Yes" ## 4 missing
zipcode_served$psychmissing[!zipcode_served$ZIP_CODE %in% unique(ooi_long$patzip[ooi_long$OOI=="psych"])] <- "Yes" ## 3 missing
copd_codes <- c("J41", "J42", "J43", "J44",
                "491", "492", "496")
zipcode_served$copdmissing[!zipcode_served$ZIP_CODE %in% unique(ooi_long$patzip[substr(ooi_long$diag_p, 1, 3) %in% copd_codes])] <- "Yes"  ## 40 missing
sum(is.na(zipcode_served$respmissing)|is.na(zipcode_served$cardiomissing)|is.na(zipcode_served$psychmissing)) ## no missing if considering all outcomes
zipcode_served$wfmissing[!zipcode_served$ZIP_CODE %in% unique(z_wfpm$zip_code)] <- "Yes" ## 22 missing
zipcode_served$tmptmissing[!zipcode_served$ZIP_CODE %in% unique(z_temp$zip)] <- "Yes" ## 23 missing, 22 overlapping with wf
write.csv(zipcode_served, file.path(out_dir, "zipcodes_in_analysis_by_endpoint.csv"))

## get to the list of zipcodes with at least one outcome (any type) and within the three company service area and some wf and temperature data
ca_z <- zipcode_served$ZIP_CODE[zipcode_served$ZIP_CODE %in% unique(ooi_long$patzip) & 
                         is.na(zipcode_served$wfmissing)  & is.na(zipcode_served$tmptmissing)] ## 1481
...
ooi_psps$patzip <- as.numeric(ooi_psps$patzip) ## otherwise merge fails
...
adult_only <- merge(ooi_psps_Wf, z_temps, by.x = c("patzip", "serv_dt"), by.y = c("zip", "date"), all.x = TRUE)
...
#write.csv(zctalist, file.path(out_dir, "list_zcta_in_analysis.csv"), row.names = F)
##temporary codes, please delete before finalization--kept here for record only

# outcomes of interest (OOI)
cat("number of zipcodes with temperature, wildfire smoke, psps service coverage, and some adult health outcomes are", length(unique(adult_only$patzip)), "\n")

adult_cardio <- adult_only[OOI == "cardio"]
cat("number of zipcodes with temperature, wildfire smoke, psps service coverage, and some adult cardio outcomes are", length(unique(adult_cardio$patzip)), "\n")

adult_resp <- adult_only[OOI == "resp"]
cat("number of zipcodes with temperature, wildfire smoke, psps service coverage, and some adult resp outcomes are", length(unique(adult_resp$patzip)), "\n")

copd_codes <- c("J41", "J42", "J43", "J44",
                "491", "492", "496")
adult_copd <- adult_only[substr(diag_p, 1, 3) %in% copd_codes]
cat("number of zipcodes with temperature, wildfire smoke, psps service coverage, and some adult copd outcomes are", length(unique(adult_copd$patzip)), "\n")

adult_psych <- adult_only[OOI == "psych"]
cat("number of zipcodes with temperature, wildfire smoke, psps service coverage, and some adult psych outcomes are", length(unique(adult_psych$patzip)), "\n")
...
for (i in 1:length(dat)) {
  ...
  ## extract exposures and temperatures
  temp <- cbind(as.data.frame(df[case_indicator==1, lapply(.SD, mean),.SDcols = c("wf_pm25", "mean_lag0_lag5", "tmmx_C")]),
                as.data.frame(df[case_indicator==1, lapply(.SD, sd),.SDcols = c("wf_pm25", "mean_lag0_lag5", "tmmx_C")]),
                as.data.frame(df[case_indicator==1, .N]),
                outcome = df_name[i])
  names(temp)[1:7] <- c(paste(rep(c("wf_pm25", "mean_lag0_lag5", "tmmx_C"), times=2), rep(c("mean", "SD"), each=3), sep="_"), "case_days")
  bar <- rbind(bar, temp)  
  
  temp2 <-cbind(as.data.frame(df[case_indicator==1, .N, by=c("severity_customers")]),
                as.data.frame(df[case_indicator==1, .N, by=c("severity_hybrid")]), 
                as.data.frame(df[case_indicator==1, .N]),
                outcome = df_name[i])
  names(temp2)[c(2, 4, 5)] <- c("severity_customers_N", "severity_hybrid_N", "case_days")
  foo <- rbind(foo, temp2)  
  ...
  abs_vcov <- vcov(absmodel)
  hyb_vcov <- vcov(hybmodel)

  write.csv(abs_vcov, file = file.path(out_dir, paste0("vcov_absmodel_", df_name[i], ".csv")))
  write.csv(hyb_vcov, file = file.path(out_dir, paste0("vcov_hybmodel_", df_name[i], ".csv")))
  ...
  # Assign the result to a variable named results3_<dataset_name>
...
write.csv(bar, file = file.path(out_dir, "wf_tmax_among_casedays.csv"), row.names = FALSE)
write.csv(foo, file = file.path(out_dir, "psps_among_casedays.csv"), row.names = FALSE)
